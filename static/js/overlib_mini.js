...
//non standard function that offsets the menu items if they are not aligned.
function offsetMenuItemText(topMargin) {
    console.log('offset menu fired');
	if( document.all ) {
        console.log('document.all fired');
		document.getElementById('menu_item').style.posTop = topMargin + document.getElementById('logocell').offsetParent.offsetTop + document.getElementById('buttoncell').offsetParent.offsetTop;
	}else{
        console.log('no document.all');
		if(document.getElementById('logocell')) {
            console.log('logocell found');
			document.getElementById('menu_item').style.top = parseInt( topMargin + document.getElementById('overDiv').offsetParent.offsetTop + document.getElementById('logocell').offsetParent.offsetTop + document.getElementById('buttoncell').offsetParent.offsetTop ) + "px" ;
		} else {
            console.log('no logocell found');
            document.getElementById('menu_item').style.top = parseInt( topMargin + document.getElementById('overDiv').offsetParent.offsetTop + document.getElementById('feature-title-row').offsetParent.offsetTop + document.getElementById('buttoncell').offsetParent.offsetTop ) + "px" ;
        }
	}
...
//function that generate http object
function getHTTPOBJ()
{
	try
	{
...
{
	var parentIframeDoc = getIframeDocument(document.getElementById(id + '___Frame'));
	var childIframes = parentIframeDoc.body.getElementsByTagName('iframe');
	var childIframeDoc = getIframeDocument(childIframes[0]);

	return childIframeDoc.body.innerHTML.replace(/\n/g, '').replace(/<br[ \/]*>/gi, '\n').replace(/<[^>]*>/g, '').replace(/&nbsp;/g, '').replace(/^\s*/, '').replace(/\s*$/, '');
}